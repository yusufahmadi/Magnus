...
Partial Public Class FormTaffeta
    ...
    Function IsValid() As Boolean
        ...
        If Not CBool(ckIsActive.Checked) Then
            If DialogResult.No = DevExpress.XtraEditors.XtraMessageBox.Show("Set " & FormName & " Non Aktif", NamaAplikasi, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) Then
                ckIsActive.Focus()
                IsValid = False
                Exit Function
            End If
    ...
    Function IsValidOnDB() As Boolean
        ...
        If CInt(Query.ExecuteScalar("Select Count(*) From " & TableName & " Where ID<> " & Me._ID & " AND Kode ='" & txtKode.Text & "'")) > 0 Then
            DevExpress.XtraEditors.XtraMessageBox.Show("Kode sudah ada.", NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Information)
            txtKode.Focus()
            IsValidOnDB = False
            Exit Function
        End If
        If CInt(Query.ExecuteScalar("Select Count(*) From " & TableName & " Where ID<> " & Me._ID & " AND Nama ='" & txtNama.Text & "'")) > 0 Then
            DevExpress.XtraEditors.XtraMessageBox.Show("Nama sudah ada.", NamaAplikasi, MessageBoxButtons.OK, MessageBoxIcon.Information)
            txtKode.Focus()
            IsValidOnDB = False
            Exit Function
        End If
